...
### L5. 未リリースのサーバ機能を前提にしたクライアント側リファクタは、リリース確定まで着手しない
...
- 開発環境で「エンドポイントが 404 でない」ことは疎通の証明にならない。**実際の処理が最後まで通るか**を1件叩いて確かめる。バリデーションだけ通って本処理が timeout する、といった状態がありうる。
- 依存関係が読めない場合は、着手前に「この機能はいつ本番に入るか」を確認する。
