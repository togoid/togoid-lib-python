#!/usr/bin/env python3
"""
ID to Annotations Converter - コマンドラインツール

このスクリプトは TogoID の Web UI で提供されている ID からラベル・アノテーションへの
変換機能をコマンドライン上で再現します。TogoID 設定 API からデータセットごとの
アノテーション情報を取得し、GRASP GraphQL エンドポイントを通じて ID に紐付くラベルや
メタデータを解決します。

使用例:
    # NCBI Gene データセットで利用可能なアノテーション項目を表示
    python id_to_annotations.py --dataset ncbigene --list-fields

    # 2 つの ID について遺伝子同義語と正式名称を取得
    python id_to_annotations.py --dataset ncbigene --ids 672 7157 \
        --field gene_synonym --field full_name

    # アノテーション値でフィルタリングし CSV で保存
    python id_to_annotations.py --dataset ncbigene --ids 672 7157 \
        --field gene_synonym --field type_of_gene \
        --filter type_of_gene=protein-coding --format csv --output genes.csv
"""
...
# 既定のエンドポイントは Web アプリと同じ挙動になるように設定しています。
# 必要に応じて CLI オプションまたは環境変数で上書きしてください。
...
class IDToAnnotationsConverter:
